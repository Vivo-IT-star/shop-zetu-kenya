...
interface PinkProductsContextType {
...
const PinkProducts = createContext<PinkProductsContextType | null>(null);

export function PinkProductsProvider({
  children,
}: {
  ...
  const fetchInitialProducts = useCallback(async () => {
    try {
      ...
      // console.log('[PinkProducts] Fetching initial products:', {
      //   endpoint: '/api/vivo_products',
      //   handle: 'vivo',
      //   first: 250
      // });
      const response = await fetch('/api/think-pink-products?handle=pink-collection&first=250');
  ...
  const loadMoreProducts = useCallback(async () => {
    console.log('[PinkProducts] Pagination request:', {
      endpoint: '/api/think-pink-products',
    ...
    try {
      ...
      const response = await fetch(`/api/think-pink-products?${params}`);
      const data: unknown = await response.json();
  ...
  return (
    <PinkProducts.Provider value={{
      ...
      {children}
    </PinkProducts.Provider>
  );
}

export function usePinkProducts() {
  const context = useContext(PinkProducts);
  if (!context) {
    throw new Error(
      'usePinkProducts must be used within a Pink Products Provider',
    );
  }
  return context;
}