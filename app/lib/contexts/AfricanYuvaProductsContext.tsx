...
interface AfricanYuvaProductsContextType {
...
const AfricanYuvaProducts = createContext<AfricanYuvaProductsContextType | null>(null);

export function AfricanYuvaProductsProvider({
  children,
}: {
  ...
  const fetchInitialProducts = useCallback(async () => {
    try {
      ...
      // console.log('[AfricanYuvaProducts] Fetching initial products:', {
      //   endpoint: '/api/vivo_products',
      //   handle: 'vivo',
      //   first: 250
      // });
      const response = await fetch('/api/african-yuva_products?handle=african-yuva&first=250');
  ...
  const loadMoreProducts = useCallback(async () => {
    console.log('[AfricanYuva Products] Pagination request:', {
      endpoint: '/api/african-yuva_products',
      handle: 'african-yuva',
      first: 250,
    ...
    try {
      ...
      const response = await fetch(`/api/african-yuva_products?${params}`);
      const data: unknown = await response.json();
  ...
  return (
    <AfricanYuvaProducts.Provider value={{
      products, 
      loading, 
      error, 
      pageInfo, 
      ...
      {children}
    </AfricanYuvaProducts.Provider>
  );
}

export function useAfricanYuvaProducts() {
  const context = useContext(AfricanYuvaProducts);
  if (!context) {
    throw new Error(
      'useAfricanYuvaProducts must be used within a African Yuva Products Provider',
    );
  }
  return context;
}