...
import {usePumaProducts} from '~/lib/contexts/PumaProductsContext';
import {Link} from 'react-router';
import PumaHeroBanners from '~/components/PumaHeroBanners';
...
const PUMA_COLLECTIONS_QUERY = `#graphql
  query PumaWomenCollections {
    trainers: collection(handle: "puma-women-trainers") {
      handle
      title
      image {
        url
    ...
    tops: collection(handle: "puma-women-tops") {
      handle
      title
      image {
        url
    ...
    leggings: collection(handle: "puma-women-leggings") {
      handle
      title
      image {
        url
    ...
    sportsBras: collection(handle: "puma-womens-sports-bra") {
      handle
      title
      image {
        url
    ...
    jackets: collection(handle: "puma-women-jackets") {
      handle
      title
      image {
        url
     ...
     hoodiesAndSweatshirts: collection(handle: "puma-hoodies-sweatshirts") {
      handle
      title
      image {
        url
...
export async function loader({context, request, params}: LoaderFunctionArgs) {
  // fetch collections
  const result = await context.storefront.query(PUMA_COLLECTIONS_QUERY);
...
async function loadCriticalData({context}: LoaderFunctionArgs) {
  ...
  const featuredProductIds = [
    'gid://shopify/Product/8962800550107',
    'gid://shopify/Product/8957205020891',
    'gid://shopify/Product/8969549054171',
    'gid://shopify/Product/8942239056091',
    'gid://shopify/Product/8543594479835',
    'gid://shopify/Product/7542427877595',
    'gid://shopify/Product/7542446293211',
    'gid://shopify/Product/7853102530779',
    'gid://shopify/Product/8948302643419',
    'gid://shopify/Product/8957205840091',
  ];
...
const ShopPuma = () => {
  const {collections, topSellingProducts, topSellingProductsByID} =
  ...
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
    ...
    const availableCategories = React.useMemo<Category[]>(() => {
      const set = new Set<Category>();
  
      products.forEach((product) => {
        product.tags?.forEach((tag) => {
          const upper = tag.toUpperCase();
          if (upper in CATEGORY_TAGS) {
            set.add(upper as Category);
          }
        });
      });
  ...
  if (loading) {
    return <div className="text-center my-8">Loading Puma products...</div>;
  }
  ...
  if (!products.length) {
    return <div className="text-center my-8">Puma Products Coming Soon</div>;
  }
  ...
  return (
    <div className="flex flex-col w-full">
     <div className="w-full mt-52 md:mt-48 ">
      ...
      {/* Collections Section */}
      <div className="bg-white z-20 pb-12">
        {collections?.length > 0 && (
          <div className="px-4 mt-10">
            <div className="text-center font-semibold text-lg lg:text-2xl my-10 lg:my-0 ">
              PUMA WOMEN FAVS
            </div>
            <div className="grid grid-cols-2 lg:scale-80 md:grid-cols-6 gap-6 ">
              {collections.map((col) =>
                col ? (
                  <div
                    key={col.handle}
                    className="flex flex-col bg-white rounded-lg shadow hover:shadow-lg hover:scale-110 transition overflow-hidden cursor-pointer"
                    onClick={() =>
                  ...
                  >
                    ...
                    {/* <div>
                      <h3 className="text-center text-sm mt-2 font-medium p-2">
                        {col.title}
                      </h3>
                    </div> */}
                  </div>
      ...
      <div className="z-20 relative">
        <Link to="/shop/puma" className="relative block">
          {/* Desktop Image */}
          <Image
            src="https://cdn.shopify.com/s/files/1/0621/5162/2875/files/PumaDesktopNewYearNewGear.avif?v=1767685788"
          ...
          {/* Mobile Image */}
          <img
            src="https://cdn.shopify.com/s/files/1/0621/5162/2875/files/PumaMobileNewYearNewGear.avif?v=1767685788"
            alt="New Year New Gear mobile"
            className="block md:hidden w-full"
          />

          {/* 🔥 TEXT OVERLAY */}
          <div className="hidden absolute inset-0 lg:flex flex-col justify-center items-end px-6 md:-right-20">
            <div className="text-left max-w-xl w-full">
              <h1 className="text-white text-3xl md:text-5xl font-extrabold tracking-wide">
                PUMA NEW YEAR. NEW GEAR.
              </h1>

              <p className="mt-3 text-white text-sm md:text-lg tracking-wide">
                CRUSH YOUR NEW YEAR’S FITNESS GOALS
              </p>

              <div className="mt-6 flex gap-4 flex-wrap">
                <Link to="/collections/puma-men" className="relative block">
                  <span className="bg-white text-black px-6 py-3 text-sm font-semibold tracking-wide">
                    MEN&apos;S COLLECTION
                  </span>
                </Link>

                <Link to="/collections/puma-women" className="relative block">
                  <span className="bg-white text-black px-6 py-3 text-sm font-semibold tracking-wide">
                    WOMEN&apos;S COLLECTION
                  </span>
                </Link>
        ...
        {/* Mobile Overay Text */}
        <div className="lg:hidden flex flex-col justify-center items-center mb-8 md:-right-20">
          <div className="text-center max-w-xl w-full">
            <h1 className="text-black text-3xl md:text-5xl font-extrabold tracking-wide">
              NEW YEAR. NEW GEAR.
            </h1>

            <p className="mt-3 text-black text-sm md:text-lg tracking-wide">
              CRUSH YOUR NEW YEAR’S FITNESS GOALS
            </p>

            <div className="mt-6 flex items-center justify-center gap-2 flex-wrap">
              <Link to="/collections/puma-men" className="relative block">
                <span className="bg-black text-white px-4 py-3 text-sm font-semibold tracking-wide">
                  MEN
                </span>
              </Link>
      ...
      {/* <div className="text-center text-white font-semibold text-lg lg:text-2xl mt-8 lg:mt-0">
        TOP SELLING
      </div>
 */}
      ...
      <div className="md:hidden flex justify-center">
        ...
        >
          ...
          <div className="md:hidden flex fixed items-center justify-between top-38 h-14 z-50 bg-white shadow-sm left-1/2 transform -translate-x-1/2 w-full max-w-lg px-4">
            <h1 className="text-lg font-bold truncate">Puma</h1>
