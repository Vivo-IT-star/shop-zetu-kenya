...
import {useVivoProducts} from '~/lib/contexts/VivoProductsContext';
import {Link} from 'react-router';
import VivoHeroBanners from '~/components/VivoHeroBanners';
...
import {NewInProductsCarousel} from '~/components/topSellingProducts';

const SHOPVIVO_COLLECTIONS_QUERY = `#graphql
  query ShopVivoCollections {
    ...
    vivoOuterWear: collection(handle: "vivo-bottoms") {
      handle
      title
      image {
        url
    ...
    vivoBottoms: collection(handle: "vivo-outerwear-1") {
      handle
      title
      image {
        url
...
export async function loader({context, request, params}: LoaderFunctionArgs) {
  // fetch collections
  const result = await context.storefront.query(SHOPVIVO_COLLECTIONS_QUERY);
...
async function loadCriticalData({context}: LoaderFunctionArgs) {
  ...
  const featuredProductIds = [
    'gid://shopify/Product/9112619712731',
    'gid://shopify/Product/9112619679963',
    'gid://shopify/Product/9112619614427',
    'gid://shopify/Product/9075455262939',
    'gid://shopify/Product/9075455328475',
    'gid://shopify/Product/9075455295707',
    'gid://shopify/Product/9075455230171',
    'gid://shopify/Product/9112619647195',
    'gid://shopify/Product/9115087241435',
    'gid://shopify/Product/9115086553307',
  ];
...
const ShopVivo = () => {
  ...
  console.log('Top selling products by ID:', topSellingProductsByID);

  const {products, loading, error, pageInfo, loadMoreProducts, isLoadingMore} =
    useVivoProducts();

  // console.log('Vivo products loaded:', products);
                ...
                col ? (
                  ...
                  >
                    {col.image?.url ? (
                      <Image
                        data={col.image}
                        className="w-full h-full hover:scale-110 duration-700 object-cover"
 ...
 */}
      ...
      <div className="flex flex-row gap-4 w-full min-h-screen relative overflow-hidden">
        ...
        <div className="hidden md:flex w-[310px] ">
          <div className="fixed top-40 2xl:top-45h-[450px] 2xl:h-[600px] overflow-y-auto p-4">
            <FiltersSidebar
